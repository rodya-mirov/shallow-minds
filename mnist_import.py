...
def get_mnist_nice():
    training_X, training_Y_old = load_mnist(dataset='training', path='./Datasets/MNIST/')
    test_X, test_Y_old = load_mnist(dataset='testing', path='./Datasets/MNIST/')

    s = training_X.shape
    training_X = reshape(training_X, (s[0], s[1]*s[2]))
    
    s = test_X.shape
    test_X = reshape(test_X, (s[0], s[1]*s[2]))
    
    training_Y = zeros( (len(training_Y_old), 10) )
    
    for i in range(0, len(training_Y)):
        training_Y[i, training_Y_old[i]] = 1
    
    test_Y = zeros( (len(test_Y_old), 10) )
    
    for i in range(0, len(test_Y)):
        test_Y[i, test_Y_old[i]] = 1
